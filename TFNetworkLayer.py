...
# COPY LAST HIDDEN LAYER

@contextlib.contextmanager
def _name_scope_for_concat_src_last_hidden_layers(src_layers, postfix):
  """
  :param list[LayerBase] src_layers:
  :param str postfix:
  :return: yields scope via reuse_name_scope()
  ...
  assert src_layers
  if len(src_layers) == 1:
    name_scope = src_layers[0].network.get_layer("readout").get_absolute_name_scope_prefix() + postfix
  else:
    base = src_layers[0].network.get_layer("readout").network.get_absolute_name_scope_prefix()
    name = "concat_" + "_".join([l.network.get_layer("readout").tf_scope_name for l in src_layers])
    name_scope = base + name + "/" + postfix
  from TFUtil import reuse_name_scope
  with reuse_name_scope(name_scope, absolute=True) as scope:
    yield scope

def concat_sources_last_hidden(src_layers):
  """
  :param list[LayerBase] src_layers:
  :return: data with placeholders set
  :rtype: Data
  ...
  assert src_layers, "need source layers"
  if len(src_layers) == 1:
    return src_layers[0].get_sub_layer("readout").output.copy()
  assert False, "only one source layer allowed"


def get_concat_sources_last_hidden_data_template(src_layers, name=None):
  """
  This just creates a template :class:`Data` instance,
  without creating any real TF tensors.
  :func:`conact_sources` (and related) are the equivalent functions
  ...
  assert src_layers, "need source layers"
  if len(src_layers) == 1:
    return src_layers[0].get_sub_layer("readout").output.copy(name=name)
  assert False, "only one source layer allowed"

def concat_sources_last_hidden_with_opt_dropout(src_layers, dropout=0, dropout_noise_shape=None):
  """
  :param list[LayerBase] src_layers:
  :param float dropout: will be applied if train_flag is set
  :param tuple|list|dict|None dropout_noise_shape:
  ...
  """
  assert src_layers, "need source layers"
  data = concat_sources_last_hidden(src_layers)
  network = src_layers[0].get_sub_layer("readout").network
  if network.train_flag is False:
    # If we know that we are not training, we always disable dropout.
    dropout = 0
  if not dropout:
  ...
  if isinstance(dropout_noise_shape, dict) or not dropout_noise_shape:
    ...
    # All dynamic dimensions (batch,time) will use the same dropout-mask broadcasted.
    dropout_noise_shape = data.get_bc_shape(dropout_noise_shape)
  cache_key = (tuple({src_layers[0].get_sub_layer("readout")}), float(dropout), tuple(dropout_noise_shape))
  if cache_key in network.concat_sources_dropout_cache:
    return network.concat_sources_dropout_cache[cache_key].copy()
  data = data.copy()
  assert 0.0 < dropout < 1.0
...
class _ConcatInputLastHiddenLayer(LayerBase):
    """
    Base layer which concatenates all incoming source layers in the feature dimension,
    and provides that as `self.input_data`, which is of type :class:`Data`.
    This is the most common thing what many layers do with the input sources.
    ...
    def __init__(self, dropout=0, dropout_noise_shape=None, mask=None, **kwargs):
      ...
      :param str|None mask: "dropout" or "unity" or None. this is obsolete and only here for historical reasons
      """
      super(_ConcatInputLastHiddenLayer, self).__init__(**kwargs)
      assert mask in ['dropout', 'unity', None], "invalid mask: %r" % mask
      if mask == "unity":
        assert not dropout
      elif mask == "dropout":
      ...
      self.input_data = None
      if self.sources:
        self.input_data = concat_sources_last_hidden_with_opt_dropout(
          self.sources, dropout=dropout, dropout_noise_shape=dropout_noise_shape)

class CopyLastHiddenLayer(_ConcatInputLastHiddenLayer):
  """
  This layer does nothing, it copies its input.
  If multiple sources are provided, they are concatenated in the feature-dim.
  """

  layer_class = "copy_last_hidden"

  def __init__(self, **kwargs):
    super(CopyLastHiddenLayer, self).__init__(**kwargs)
    self.output = self.input_data.copy(name="%s_output" % self.name)
    if len(self.sources) == 1:
      self.output_loss = self.sources[0].get_sub_layer("readout").output_loss
      if not self.dropout:
        self.output_before_activation = self.sources[0].get_sub_layer("readout").output_before_activation

  @classmethod
  def get_out_data_from_opts(cls, name, sources=(), out_type=None, n_out=None, **kwargs):
    return get_concat_sources_last_hidden_data_template(sources, name="%s_output" % name)
