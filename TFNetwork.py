...
class TFNetwork(object):
  ...
  def construct_extra_net(self, net_dict, layer_list, search_flag=None):
    ...
    for layer_name in layer_list:
      self.extra_net.construct_layer(net_dict=net_dict, name=layer_name, get_layer=extra_get_layer, check_existing=False)
    ...
    for name, layer in layer_items:
      ...
      with reuse_name_scope("loss"):
        ...
        if with_total:
          ...
          loss_rate = float(self.get_config().value("loss_rate", 0.70))
          print("Loss Rate:", loss_rate)
          for loss_obj in losses:
            if loss_obj.get_loss_value_for_objective() is not None:
              loss_weight = 0.0
              if loss_obj.name == "output/output_prob":
                loss_weight = loss_rate
              elif loss_obj.name == "ctc":
                loss_weight = 1.0
              else:
                loss_weight = (1.0-loss_rate)
              print("Loss Weight for",loss_obj.name,"is",loss_weight,"!")
              if total_loss is 0:
                total_loss = loss_weight * loss_obj.get_loss_value_for_objective()
              else:
                total_loss += loss_weight * loss_obj.get_loss_value_for_objective()
